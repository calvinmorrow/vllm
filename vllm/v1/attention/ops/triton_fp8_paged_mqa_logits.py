...
def triton_fp8_paged_mqa_logits_gfx1151(
    ...
    if kv_cache.ndim != 4 or kv_cache.shape[2:] != (1, 132):
        raise RuntimeError("gfx1151 Triton paged MQA logits requires packed FP8 KV cache")
    if kv_cache.dtype != torch.uint8 or weights.dtype != torch.float32:
        raise RuntimeError("gfx1151 Triton paged MQA logits received unsupported dtypes")
    if context_lens.ndim not in (1, 2):
        raise RuntimeError("gfx1151 Triton paged MQA logits requires rank-1 or rank-2 lengths")

    batch_size, next_n, _, head_dim = q_fp8.shape
    block_size = kv_cache.shape[1]
    if block_size % 16 or block_size % 64:
        raise RuntimeError("gfx1151 Triton paged MQA logits requires block size divisible by 64")
